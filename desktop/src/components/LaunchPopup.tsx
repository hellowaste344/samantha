...
>>>>>>> 8b37e69bd68cde7857316aa08a0ca46ea483029f
  ...
  useEffect(() => {
    ...
    const drawStrand = (points: any[], cf: string) => {
      for (let i = 0; i < points.length - 1; i++) {
        const p0 = points[i], p1 = points[i + 1];
        const z = (p0.z + p1.z) / 2;
        const t01 = (z + 1) / 2;
    ...
    const frame = () => {
      ...
      const azB = pB.reduce((s, p) => s + p.z, 0) / NODES;
      if (azA < azB) {
        drawStrand(pA, "59,130,246");
        drawStrand(pB, "96,165,250");
      } else {
        drawStrand(pB, "96,165,250");
        drawStrand(pA, "59,130,246");
      }
  ...
  // Fix 2: replaced once:true listener with a proper cleanup so it doesn't leak
  useEffect(() => {
  ...
  // Fix 3: added Escape key to close actions panel
  useEffect(() => {
    if (!showActions) return;
    const handler = (e: KeyboardEvent) => { if (e.key === "Escape") setShowActions(false); };
...
>>>>>>> 8b37e69bd68cde7857316aa08a0ca46ea483029f
        ...
        {/* Status hint */}
        {actionStatus && (
          <p className="lp-status-hint" role="status">{actionStatus}</p>
        )}
